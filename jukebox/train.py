...
from jukebox.utils.radam import RAdam
...
def get_optimizer(model, hps):
    ...
    if hps.fp16_opt:
        ...
        opt = RAdam(model.parameters(), lr=hps.lr, weight_decay=hps.weight_decay, betas=betas, eps=hps.eps)
    # lr scheduler
...
def train(model, orig_model, opt, shd, scalar, ema, logger, metrics, data_processor, hps):
    ...
    orig_model.train()
    if hps.prior:
        _print_keys = dict(l="loss", bpd="bpd", g_l="gen_loss", p_l="prime_loss")
    else:
