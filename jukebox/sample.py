...
def sample_partial_window(zs, memory, labels, sampling_kwargs, level, prior, tokens_to_sample, hps):
    z = zs[level]
    n_ctx = prior.n_ctx
    ...
    z_last, _ = sample_single_window(zs, memory, labels, sampling_kwargs, level, prior, start, hps)
    return z_last

# Sample a single window of length=n_ctx at position=start on level=level
def sample_single_window(zs, memory, labels, sampling_kwargs, level, prior, start, hps):
    ...
    for z_i, z_conds_i, y_i in zip(z_list, z_conds_list, y_list):
        z_samples_i, memory = prior.sample(n_samples=z_i.shape[0], z=z_i, memory=memory, z_conds=z_conds_i, y=y_i, **sampling_kwargs)
...
def sample_level(zs, labels, sampling_kwargs, level, prior, total_length, hop_length, hps):
    ...
    if total_length >= prior.n_ctx:
        for start in get_starts(total_length, prior.n_ctx, hop_length):
            zs, memory = sample_single_window(zs, memory, labels, sampling_kwargs, level, prior, start, hps)
    else:
        zs = sample_partial_window(zs, memory, labels, sampling_kwargs, level, prior, total_length, hps)
    return zs
...
def save_samples(model, device, hps, sample_hps):
    ...
    # For the 1b_lyrics top level, labeller will look up artist and genres in v3 set (after lowercasing).lower_batch_size = 16
    ...
    sampling_kwargs = [dict(temp=.99, fp16=True, max_batch_size=lower_batch_size, chunk_size=lower_level_chunk_size),
                       dict(temp=0.99, fp16=True, max_batch_size=lower_batch_size, chunk_size=lower_level_chunk_size),
                       dict(temp=0.98, fp16=True, max_batch_size=max_batch_size, chunk_size=chunk_size)]
    ...
    if sample_hps.mode == 'ancestral':
        ...
        if sample_hps.prompt_length_in_seconds is not None:
            ...
            duration = None
        zs = load_codes(sample_hps.codes_file, duration, priors[-1], hps)
